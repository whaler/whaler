...
const extend = require('dockerode/lib/util').extend;
 ...
 * @constructor
 */
function Storage(name, dir = '/var/lib/whaler/storage') {
    if (name) {
...
Storage.prototype.update = function(id, data, callback) {
    ...
    this.db.update(id, { $set: data }, { multi }, (err, numReplaced) => {
        if (err) {
            return callback(err);
        }
        callback(null);
...
Storage.prototype.remove = function(id, callback) {
    ...
    this.db.remove(id, { multi }, (err, numRemoved) => {
        if (err) {
            return callback(err);
        }
