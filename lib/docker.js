...
const dockerIgnore = require('@balena/dockerignore');
...
function followProgress (docker, stream, onFinished) {
    if ('quiet' == process.env.WHALER_DOCKER_PROGRESS) {
        docker.modem.followProgress(stream, (err, output) => {
            onFinished(extractBuildError(err, output), output);
        });
        ...
        if ('interactive' === process.env.WHALER_FRONTEND && process.stderr.isTTY) {
        ...
        docker.modem.followProgress(stream, (err, output) => {
            response = {
                err: extractBuildError(err, output),
                output: output
            };
        }, (event) => {
            events.push(event);
...
/**
 * docker-modem's followProgress only reports transport-level errors; a failed
 * build (e.g. base image pull failure) instead ends the stream normally with
 * an `{ error: ... }` event, so it has to be detected here.
 *
 * @param err
 * @param output
 * @returns {Error|null}
 */
function extractBuildError (err, output) {
    if (err) {
        return err;
    }
    ...
    if (Array.isArray(output)) {
        for (let i = output.length - 1; i >= 0; i--) {
            if (output[i] && output[i].error) {
                return new Error(output[i].error);
            }
        }
    }
...
function printEvent (event) {
    if (event.error) {
        ...
        process.stderr.write(event.id + ': ' + event.status + ' ' + (event.progress || formatProgressDetail(event.progressDetail)) + '\n');
    } else {
        if (event.stream) {
...
/**
 * Newer Docker API versions stopped sending the pre-rendered `progress` bar
 * string on pull/push layer events, only `progressDetail: {current, total}`.
 *
 * @param detail
 * @returns {string}
 */
